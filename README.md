...
Десктопное приложение на **PySide6**, которое ищет оптимальный порядок перемножения цепочки матриц с помощью генетического алгоритма. Результаты визуализируются на графике сходимости и в виде дерева умножения.
...
Задача — найти порядок перемножения с **минимальной стоимостью**. Точное решение считается динамическим программированием (`calculate_min_cost`), а генетический алгоритм ищет хорошее приближение за приемлемое время.
...
- Панель результатов: лучшая стоимость, отношение к целевому, поколение сходимости, порядок умножения.
...
|---|---|---|
| Размер популяции | 200 | Число особей в поколении |
| Число поколений | 100 | Сколько поколений эволюционировать |
| Вероятность мутации | 0.01 | Шанс изменения гена у потомка |
| Вероятность скрещивания | 0.8 | Шанс скрещивания пары родителей |
...
└── out/docs/diagrams/               # UML-диаграммы архитектуры
    ├── 01_application_shell/
    ├── 02_backend/
    ├── 03_input_config/
    ├── 04_results_tree/
    └── 05_full_overview/
